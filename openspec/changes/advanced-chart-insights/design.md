...
- 변경 후: 추가로 `correlation`, `distribution`, `heatmap`, `flow`, `comparison`, `mixed_axis`, `radar` 등 11개 인사이트를 지원하는 프롬프트를 구성합니다.
  - JSON 내 `agg_func`, `value`, `group_by` 외에도 복수 데이터(`values: []`), `source_col`, `target_col` (Sankey용) 등을 유연하게 포함하도록 프롬프트 구조화.
...
2. `insight_recommendation.py`의 `generate_chart_prompt`가 `{"analysis_type": "scatter_visualmap", ...}` Intent 생성
